...
class _SevenClassState extends State<SevenClass> {
  ...
  Future<void> _addData() async {
    if (_titleController.text.isNotEmpty &&
      ...
      await _firestore.collection('seven').add({
  ...
  Future<void> _updateData(String documentId) async {
    if (_titleController.text.isNotEmpty &&
      ...
      await _firestore.collection('seven').doc(documentId).update(updatedData);
  ...
  Widget _buildDataList() {
    return Expanded(
      child: StreamBuilder<QuerySnapshot>(
        stream: _firestore.collection('seven').snapshots(),
                          ...
                          ? GestureDetector(
                              ...
                              child: ClipRRect(
                                ...
                                child: Image.network(
                                  imageUrl,
                                  height: 120,
                                  width: 80, // Adjust the hSeven as needed
                                ),
                              ),
                            )
                          : Container(),
  ...
  Future<void> _deleteData(String documentId) async {
    await _firestore.collection('seven').doc(documentId).delete();
  }
