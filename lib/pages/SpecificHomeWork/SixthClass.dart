...
class SixClass extends StatefulWidget {
  @override
  _SixClassState createState() => _SixClassState();
}

class _SixClassState extends State<SixClass> {
  ...
  Future<void> _addData() async {
    if (_titleController.text.isNotEmpty &&
      ...
      await _firestore.collection('six').add({
  ...
  Future<void> _updateData(String documentId) async {
    if (_titleController.text.isNotEmpty &&
      ...
      await _firestore.collection('six').doc(documentId).update(updatedData);
  ...
  Widget _buildDataList() {
    return Expanded(
      child: StreamBuilder<QuerySnapshot>(
        stream: _firestore.collection('six').snapshots(),
                          ...
                          ? GestureDetector(
                              ...
                              child: ClipRRect(
                                ...
                                child: Image.network(
                                  imageUrl,
                                  height: 120,
                                  width: 80, // Adjust the hsix as needed
                                ),
                              ),
                            )
                          : Container(),
  ...
  Future<void> _deleteData(String documentId) async {
    await _firestore.collection('six').doc(documentId).delete();
  }
